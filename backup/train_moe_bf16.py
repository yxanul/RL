...
def checkpoint_path(output_dir, step):
    ...
    return os.path.join(output_dir, f"step-{step:06d}.pt")


def best_checkpoint_path(output_dir):
    """Path for the best checkpoint"""
    return os.path.join(output_dir, "best.pt")


def save_checkpoint(path, model, optimizer, scheduler, scaler, global_step, best_eval_loss=None, extra=None):
...
def load_checkpoint(path, model, optimizer, scheduler, scaler, device):
    ...
    return global_step, best_eval_loss, extra
...
def extract_moe_metrics(model, outputs=None):
    ...
    # Add current MoE configuration info (logged once)
    if not hasattr(extract_moe_metrics, '_config_logged'):
        extract_moe_metrics._config_logged = True
        if hasattr(model, 'config'):
            metrics['config/capacity_factor'] = getattr(model.config, 'capacity_factor', 1.25)
            metrics['config/num_experts'] = getattr(model.config, 'num_experts', 16)
            metrics['config/experts_per_token'] = getattr(model.config, 'num_experts_per_token', 2)
    ...
    for i, layer in enumerate(model.layers):
        if hasattr(layer, 'moe_layer'):
            ...
            # Try to extract routing scores for entropy calculation
            # Check multiple possible locations where Tutel might store gate outputs
            gate_logits = None
            for attr_name in ['gates_s', 'gate_scores', 'logits', 'gate_logits']:
                if hasattr(moe_layer, attr_name):
                    val = getattr(moe_layer, attr_name)
                    if val is not None and torch.is_tensor(val) and val.numel() > 0:
                        gate_logits = val
                        break

            # Also try to find the gate module itself
            if gate_logits is None:
                for name, module in moe_layer.named_modules():
                    if 'gate' in name.lower() and hasattr(module, 'weight'):
                        # We have the gate linear layer, but need the forward output
                        # Store reference for potential hook
                        metrics[f'layer_{i}/has_gate_module'] = 1.0
                        break

            if gate_logits is not None:
                # Calculate router entropy (measure of routing diversity)
                # Higher entropy = more uniform routing, lower = more specialized
                with torch.no_grad():
                    routing_probs = F.softmax(gate_logits.float(), dim=-1)
                    # Shannon entropy: -sum(p * log(p))
                    entropy = -(routing_probs * (routing_probs + 1e-10).log()).sum(dim=-1).mean()
                    metrics[f'layer_{i}/routing_entropy'] = entropy.item()

                    # Normalized entropy (0 to 1, where 1 = perfectly uniform)
                    max_entropy = np.log(routing_probs.shape[-1])  # log(num_experts)
                    normalized_entropy = entropy.item() / max_entropy
                    metrics[f'layer_{i}/routing_entropy_normalized'] = normalized_entropy

                    # Router confidence (max probability per routing decision)
                    max_probs = routing_probs.max(dim=-1)[0]
                    metrics[f'layer_{i}/router_confidence_mean'] = max_probs.mean().item()
                    metrics[f'layer_{i}/router_confidence_std'] = max_probs.std().item()
...
def evaluate(model, dataloader, device, num_eval_steps=50, precision='bf16'):
    ...
    # Match training autocast to avoid precision mismatch
    if precision == 'bf16':
        autocast_dtype = torch.bfloat16
    elif precision == 'fp16':
        autocast_dtype = torch.float16
    ...
    for i, batch in enumerate(dataloader):
        ...
        tokens = batch_size * max(0, seq_len - 1)
...
def main():
    ...
    if args.resume_from and os.path.exists(args.resume_from):
        ...
        global_step, best_eval_loss, extra = load_checkpoint(
            args.resume_from, model, optimizer, scheduler, scaler, device
        )
        tokens_processed = extra.get('tokens_processed', 0)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if (batch_idx + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % args.eval_steps == 0 and parallel_env.global_rank == 0:
                ...
                print(f"\n[Eval at step {global_step}] Loss: {eval_loss:.4f} | Perplexity: {eval_ppl:.2f}")
                ...
                if eval_loss < best_eval_loss:
                    ...
                    save_path = best_checkpoint_path(args.output_dir)
                    save_checkpoint(
                        save_path, model, optimizer, scheduler, scaler,
                        global_step, best_eval_loss,
                        extra={'tokens_processed': tokens_processed}
                    )
                    print(f"Saved best checkpoint (loss: {eval_loss:.4f}) to {save_path}")

            # Regular checkpoint saving
            if global_step % args.save_steps == 0 and parallel_env.global_rank == 0:
                save_path = checkpoint_path(args.output_dir, global_step)
                save_checkpoint(
                    save_path, model, optimizer, scheduler, scaler,
                    global_step, best_eval_loss,
                    extra={'tokens_processed': tokens_processed}
                        ...
                        if layer_stats:
                            for li, st in enumerate(layer_stats):
                                ...
                                if total > 0:
                                    ...
                                    drp = st.get("dropped", np.zeros_like(tok))
                                    ...
                                    uniform_p = 1.0 / len(p)
                                    KL = float((p * (np.log(p + eps) - np.log(uniform_p))).sum())
                                    # Normalized entropy (0-1, where 1 = perfectly uniform)
                                    max_H = np.log(len(p))
                                    norm_H = H / max_H if max_H > 0 else 0

                                    wandb_metrics[f'routing/layer_{li}/entropy'] = H
